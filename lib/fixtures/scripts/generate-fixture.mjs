...
function buildFixture(seed) {
  ...
  // Order blobs: spec 034 / T052 / FR-012 — emit two blobs so the
  // FR-012 regression test has the two-order shape it needs (one
  // previous, one next). The existing generator emitted a single
  // blob at orders[0]; we now emit a second blob at orders[1]
  // representing the previous order. The order blobs are sorted
  // latest-first so orders[0] stays the (next) order that every
  // existing parser / assertion expects — the previous blob lands
  // at orders[1] as a strictly-less-recent anchor.
  const nextDeliveryDate = addDays(refDate, -1);
  const previousDeliveryDate = addDays(refDate, -8);

  const buildOrderBlob = (
    deliveryDate,
    booker,
    deliveredTo,
    slot,
  ...
  const orderBlobPath = `orders/${nextDeliveryDate}/${orderNumber}.json`;
  const nextOrderItems = buildOrderItems();

  // Previous-order identity uses a deterministic suffix (-PREV) so
  // the two blobs never collide on the orderNumber path. The
  // previous blob mirrors the next blob's item catalogue so the
  // FR-012 pipeline assertions can spot a duplicate-name item
  // across deliveries and assert the per-delivery sub-heading +
  // previous-badging works as expected.
  const previousOrderNumber = `${delivery_template.order_number_prefix}${Math.floor(rand() * 9000) + 1000}-PREV`;
  const previousOrderBlobPath = `orders/${previousDeliveryDate}/${previousOrderNumber}.json`;
  const previousBooker = `${pick(name_pool.first_names)} ${pick(name_pool.last_names)}`;
  const previousDeliveredTo = pick(location_pool);
  const previousDeliverySlot = pick(delivery_template.delivery_slots);
  const previousOrderItems = buildOrderItems();

  const nextOrderBlob = buildOrderBlob(
    nextDeliveryDate,
    booker,
    deliveredTo,
    deliverySlot,
  ...
  const previousOrderBlob = buildOrderBlob(
    previousDeliveryDate,
    previousBooker,
    previousDeliveredTo,
    previousDeliverySlot,
    previousOrderNumber,
    previousOrderBlobPath,
    previousOrderItems,
  );

  // Sort latest-first: the next delivery (deliveryDate == refDate - 1)
  // is always more recent than the previous delivery (refDate - 8), so
  // orders[0] is guaranteed to be the next blob.
  const orderBlobs = [nextOrderBlob, previousOrderBlob].sort((a, b) =>
  ...
  // Spec 034 / T052 — emit both order blobs and a merged
  // deliveryWindows list (next + previous) so the FR-012
  // regression test sees two classified deliveries.
  const fixture = {
    orders: orderBlobs,
    coverage,
    summary,
