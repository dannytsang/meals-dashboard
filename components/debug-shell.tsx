...
import { ProductResolutionDebugPanel } from './product-resolution-debug-panel';
import { RuntimeContextDebugPanel } from './runtime-context-debug-panel';

interface DebugPanelMeta {
  kind: 'items-by-category' | 'runtime-context' | 'blob-read-freshness' | 'product-resolution';
  title: string;
  description: string;
}
...
const PANELS: readonly DebugPanelMeta[] = [
  ...
  {
    kind: 'blob-read-freshness',
    title: 'Blob Read Freshness',
    description: 'Diagnostic for the live blob read path: pointer, manifest, summary, coverage, order and product read statuses plus freshness timestamps.',
  },
  {
                ...
                <div style={{ padding: '0 1rem 1rem' }}>
                  ...
                  */}
                  <div key={`${panel.kind}-${refreshKey}`}>
                    ...
                    {panel.kind === 'runtime-context' && <RuntimeContextDebugPanel />}
                    {panel.kind === 'blob-read-freshness' && <BlobReadFreshnessDebugPanel />}
                    {panel.kind === 'product-resolution' && <ProductResolutionDebugPanel />}
                  </div>
                </div>
              )}
            </section>
